...
const userId = "FT029182";
const password = "Bangaru@944";
const actId = "FT029182";
const source = "API";
...
const logStreamName = 'natural_gas_mini_15min_maa';

const utcNow = moment.utc();

// Define the timezone for Indian Standard Time (IST)
const istNow = utcNow.tz('Asia/Kolkata');
...
const place_order = async (trading_symbol, jKey, direction, qty,is_sl_order = false) => {
  console.log("qty in place order -------", qty * LOT_SIZE);
...
//   console.log("response from search_script ", resp?.data?.values);
  ...
  if (last_close > ema15[0] && rsi[0] >= 59) {
  ...
  } else if (last_close < ema15[0] && rsi[0] <= 41) {
...
const start = async () => {
  try {
    ...
    if (positions?.data?.stat === "Not_Ok" || !has_natgas_in_positions) {
      ...
      console.log("in check for exit-----------", positions);
      let curr_qty = 0;
      let tsym = "";
